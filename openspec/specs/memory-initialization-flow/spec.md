# memory-initialization-flow Specification

## Purpose
定义用户在资料编辑页触发的「清空 + 重新初始化记忆」两步流程，涵盖 UI 入口、危险确认、清空调用、初始化表单、提交、toast、记忆开关控制可见性。原始 spec 来自 change `add-memory-init-and-clear`（归档于 2026-06-12）。
