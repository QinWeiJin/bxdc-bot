# Capability: conversation-file-isolation

> **Purpose**: Conversation-level file access control. Each conversation maintains its own `enabled_files` list, isolating file operations to only files explicitly permitted for that conversation.

## Purpose

TBD - created from delta spec conversation-file-isolation.
