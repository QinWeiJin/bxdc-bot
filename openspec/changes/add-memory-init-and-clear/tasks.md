...
- [ ] 1.1 联调运维确认 mem0 服务已部署 `POST /deletemem` 接口，curl 实测 `{ userid: "test-user-001", sentence: "" }` 返回 `{ code: 200, ... }`
- [ ] 1.2 验证「不存在的 userid」幂等返回 200 而非报错
- [ ] 1.3 验证「跨用户隔离」：`POST /deletemem` 指定 user A，不影响 user B 的记忆

## 2. agent-core: MemoryService 增加删除与状态方法

- [ ] 2.1 在 `backend/agent-core/src/mem/memory.service.ts` 增加 `async deleteAllMemories(userId: string)`，POST 到 `${mem0Url}/deletemem` 携带 `{ userid, sentence: '' }`，按现有 `msearch`/`madd` 错误处理风格 catch + logger
- [ ] 2.2 在同一文件复用 `readMem0EnabledFlag()`，暴露 `async getMemoryStatus(userId: string): Promise<{ enabled: boolean; hasMemory: boolean }>`，其中 `hasMemory` 调一次 `getAllMemories(userId, 1)` 数组非空判定
- [ ] 2.3 删除/状态接口在 `MEM0_ENABLED=false` 时 MUST NOT 真正请求 mem0，直接返回 `{ enabled: false, hasMemory: false }`（沿用现有 `readMem0EnabledFlag` 语义）

## 3. agent-core: MemoryController 暴露新端点
...
- [ ] 3.2 在同一 controller 增加 `GET /memory/status?userId=xxx` 端点，返回 `getMemoryStatus` 结果
...
- [ ] 4.1 在 `frontend/src/composables/` 新建 `useMemory.ts`，导出 `getMemoryStatus(userId): Promise<{ enabled; hasMemory }>`、`deleteUserMemory(userId): Promise<void>`、`addUserMemory(userId, text): Promise<void>`，基础 URL 沿用现有 `useApiBase()` 或 `import.meta.env.VITE_API_BASE` 模式
