# async-task-polling-completion

## Purpose

定义异步轮询类 skill 的**终态判定机制**：从 pollEndpoint 接口的真实返回值自动推断任务调用状态，作为用户配置 `completionJsonPath` / `completionValue` 的兜底；当用户未配置或配置错误时，系统仍能识别 SUCCESS / FAILED 等终态，避免通知中心永远卡 99%。
