# frontend-gateway-parsing-bridge

## Purpose

衔接规范：把 wgj 已落地的后端解析能力（`FileParserRouter` + `WordParser` + `TxtMdParser` + `PyParser`）暴露给前端 / 模块 5 `file_detail` / 后续 LLM 注入。本 capability **不重复定义解析行为**（解析行为在 `word-parser` / `word-parser` / 后续启雷 `excel-parser` spec 里），只规定"前端怎么调、结果怎么落 DB、模块 5 怎么读"。
