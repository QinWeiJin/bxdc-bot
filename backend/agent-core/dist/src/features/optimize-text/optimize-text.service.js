...
exports.OptimizeTextService = void 0;
...
const DEFAULT_PROMPTS = {
    ...
    api_parameter_contract: {
        systemPrompt: "你是 JSON Schema 专家。修正 JSON 语法错误（补引号、补逗号、修正括号），补充字段的 description/type/enum。只返回严格 JSON 对象：{\"optimizedText\":\"...\",\"explanation\":\"...\"}，optimizedText 必须是合法的 JSON Schema 字符串。",
        userPromptTemplate: "修正以下 JSON Schema 的语法错误并补充字段描述。\n\n当前文本：\n{{currentText}}\n{{context}}",
    },
    api_async_poll: {
...
class OptimizeTextService {
    ...
    async optimize(fieldId, currentText, context) {
        try {
    ...
    async fetchPrompt(fieldId) {
        try {
            const response = await axios_1.default.get(`${GATEWAY_URL}/api/skills/text-prompts/${fieldId}`, {
                headers: {
...
exports.OptimizeTextService = OptimizeTextService;
//# sourceMappingURL=optimize-text.service.js.map